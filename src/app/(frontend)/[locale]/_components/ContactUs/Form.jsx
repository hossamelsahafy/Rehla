...
const Form = ({ form, buttons, submit, market, s, m }) => {
  ...
  return (
      ...
      <div className="flex flex-col justify-center items-center mt-16  text-white w-full py-10 px-4">
        <div className="uppercase futura text-lg max-sm:text-sm leading-[28.16px] font-medium text-white/70 w-full max-w-2xl">
          {Object.entries(form).map(([key, label]) => (
            <div key={key} className="flex flex-col justify-center items-start w-full mb-5 sm:mb-6">
    ...
    transition-all duration-300 hover:bg-white hover:text-black"
      ...
      <div className="flex flex-col mb-10 lg:mt-24 justify-center items-start text-white whitespace-nowrap">
        <p className="uppercase font-futura text-xl lg:text-2xl max-sm:text-sm leading-[160%] font-medium text-white mb-6">
          {s}
        </p>
        ...
        <div className="hidden lg:flex flex-1 flex-col mt-10 h-full">
          <p className="uppercase font-jost text-xl lg:text-2xl leading-[28.16px] font-medium text-white/70 group-focus-within:text-white">
            {m}
          </p>
          <textarea
            rows={17}
            className="w-full -mt-3 bg-transparent border-b-2 border-white/30 text-white placeholder-white/30 
    focus:outline-none transition-all ease-in-out duration-300 py-2
    focus:border-image-slice-[1] border-gradient-focus focus:border-b-2 resize-none
    sm:rows-4 md:rows-5 lg:rows-17"
          />
        </div>
      </div>
    </div>
